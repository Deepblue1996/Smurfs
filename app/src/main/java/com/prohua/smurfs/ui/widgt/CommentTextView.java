...
public class CommentTextView extends android.support.v7.widget.AppCompatTextView {
    ...
    private CommentModel commentModels;
    ...
    public boolean onTouchEvent(MotionEvent event) {
        ...
        if (action == MotionEvent.ACTION_DOWN) {
            ...
            float distanceX = Math.abs(lastX - event.getRawX());
            float distanceY = Math.abs(lastY - event.getRawY());
        ...
        if (link.length != 0) {
            if (action == MotionEvent.ACTION_DOWN) {
                ...
                if (mStart >= 0 && mEnd >= mStart) {
                    buffer.setSpan(new BackgroundColorSpan(Color.parseColor("#f1f1f1")), mStart, mEnd,
    ...
    public CommentModel getCommentModels() {
        return commentModels;
    }

    public void setCommentModels(CommentModel commentModels) {
