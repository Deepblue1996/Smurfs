...
public class RecyclerBanner extends FrameLayout {
    ...
    public RecyclerBanner(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        defaultDrawable.setColor(Color.argb(200,100,100,100));
    ...
    private class ReyclerAdapter extends RecyclerView.Adapter{
        ...
        public RecyclerView.ViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
            return new RecyclerView.ViewHolder(LayoutInflater.from(parent.getContext()).inflate(R.layout.first_banner_item, parent, false)) {
            };
        }
        ...
        public void onBindViewHolder(RecyclerView.ViewHolder holder, int position) {
            ...
            TextView textView = (TextView) holder.itemView.findViewById(R.id.banner_text);

            Glide.with(img.getContext())
                    .load(datas.get(position % datas.size()).getImgUrl())
                    .skipMemoryCache(true)
                    .diskCacheStrategy(DiskCacheStrategy.RESULT)
                    .into(img);
            img.setOnClickListener(view -> onPagerClickListener.onClick(datas.get(position % datas.size())));

            textView.setText(datas.get(position % datas.size()).getText());
        }
